...
/* global variables */
double tsince, jul_epoch, jul_utc, eclipse_depth = 0, phase, daynum, ax, ay, az, rx, ry, rz, squint, alat, alon;
char   calc_squint;
int    Flags = 0;
 ...
 */
std::vector<std::pair<double, double>> orbit(double epoch,
                                             double xndt2o,
                                             double xndd6o,
                                             double bstar,
                                             double xincl,
                                             ...
                                             double Tsince)
{
    ...
    // int mowdm= isFlagSet(DEEP_SPACE_EPHEM_FLAG);
    // std::cout<<"mowdm" <<mowdm <<"\n";
    // if (mowdm==1)
    //{
    // std::cout<<"inside orbit (2) debugging in progress" <<"\n";
    // states  = SDP4(tsince, &tle);
    // }
    // else
    //{
    states = SGP4(tsince, &tle);
    //}
...
double Julian_Date(struct tm* cdate)
{
    ...
    julian_date = Julian_Date_of_Year(cdate->tm_year) + DOY(cdate->tm_year, cdate->tm_mon, cdate->tm_mday) +
                  Fraction_of_Day(cdate->tm_hour, cdate->tm_min, cdate->tm_sec) + 5.787037e-06;
    return julian_date;
}
...
/* calculates satelliteâ€™s eclipse status and depth*/
...
std::vector<std::pair<double, double>> SGP4(double tsince, tle_t* tle)
{
    ...
    std::vector<double> position_temp(3);
    position_temp.at(0) = rk * ux;
    position_temp.at(1) = rk * uy;
    position_temp.at(2) = rk * uz;
    position            = position_temp;

    std::vector<double> velocity_temp(3);
    velocity_temp.at(0) = rdotk * ux + rfdotk * vx;
    velocity_temp.at(1) = rdotk * uy + rfdotk * vy;
    velocity_temp.at(2) = rdotk * uz + rfdotk * vz;
    ...
    std::vector<std::pair<double, double>> states(3);
    states.at(0).first  = position.at(0);
    states.at(0).second = velocity.at(0);
    states.at(1).first  = position.at(1);
    states.at(1).second = velocity.at(1);
    states.at(2).first  = position.at(2);
    states.at(2).second = velocity.at(2);
...
void Deep(int ientry, tle_t* tle, deep_arg_t* deep_arg)
{
    ...
    double a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, ainv2, alfdp, aqnv, sgh, sini2, sinis, sinok, sh, si, sil, day,
        ...
        xomi, xpidot, z1, z11, z12, z13, z2, z21, z22, z23, z3, z31, z32, z33, ze, zf, zm, zmo, zn, zsing, zsinh, zsini,
            ...
            do
            {
                ...
                do
                {
                    ...
                    else
                    {
                        ...
                        x2omi = xomi + xomi;
                        x2li  = xli + xli;
                        xndot = d2201 * sin(x2omi + xli - g22) + d2211 * sin(xli - g22) +
...
/* SDP 4 implementation. tsince is time since epoch in minutes, tle is a */
/* pointer to a tle_t structure with Keplerian orbital elements and position */
/* and velocity are the returning ECI satellite position and
 * /* velocity in m and m/s.*/
std::vector<std::pair<double, double>> SDP4(double tsince, tle_t* tle)
{
    ...
    velocity.at(0) = rdotk * ux + rfdotk * vx;
    velocity.at(1) = rdotk * uy + rfdotk * vy;
    velocity.at(2) = rdotk * uz + rfdotk * vz;
